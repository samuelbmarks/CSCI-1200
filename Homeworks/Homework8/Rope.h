...
private:
	...
	Node* copy_rope(Node* old_root, Node* the_parent); //copy helper function
	void destroy_rope(Node*& p); //destroy helper function
	void split_rope(int index_, std::vector<Rope>& ropes_); //helper function to split the rope
	void adjust_size(); //helper function to adjust size and weights after split
private:
	int size_;
	Node *root;
};

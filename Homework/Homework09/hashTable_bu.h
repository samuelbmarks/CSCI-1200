...
public:
  ...
  // insert will never fail because no duplicate Movie objects will be inserted (movie file does not contain duplicate movies)
  // and because table will always have space open for object to be added because of occupancy control
  void insert(KeyType const& key) {
    ...
    if (m_table[index].empty()){ //nothing in bucket, okay to add
      ...
      if (index < m_table.size()){ //if the index is less than the size of the table, then the bucket is empty and it is okay to add key
        m_table[index].push_front(key); //add key
        m_size++; //increase table size
      }
      else{ //the index is passed the end of the table, so now we must go back to the front of the table to check for an open space
  ...
  iterator find(const KeyType& key) {
    ...
    hashTable_list_itr p = std::find(m_table[index].begin(),
  ...
  iterator begin() {
    ...
    for (unsigned int i = 0; i < m_table.size(); ++i){
      if( !m_table[i].empty() ){
        hashTable_list_itr hli = m_table[i].begin();
...
private:
  void resize_table(unsigned int new_size) {
    ...
    typename std::vector<std::list<KeyType>>::iterator itr1;
    hashTable_list_itr itr2;

    for(itr1 = m_table.begin(); itr1 != m_table.end(); ++itr1){
      for (itr2 = itr1->begin(); itr2 != itr1->end(); itr2++){
        unsigned int hash_value = m_hash(*itr2);
        unsigned int index = hash_value % new_size;
        if (n_table[index].empty()){ //nothing in bucket, add that bitch
          n_table[index].push_front(*itr2);
        }
        else{
          while (!n_table[index].empty() && index < n_table.size()){ //linear probing
            index++;
          }
          if (index < n_table.size()){
            ...
            index = 0;
            while (!n_table[index].empty() && index < n_table.size()){
              index++;
            }
            n_table[index].push_front(*itr2);
          }
